<template>
  <div v-if="delivered">
    ...
    <v-list v-else two-line subheader>
      ...
      <DateRangePicker
        v-on:update-dates="updateDates"
        v-bind:initDates="initDates"
        v-if="switcher"
      ></DateRangePicker>
      <v-menu
        v-else
      ...
      >
        <v-btn @click="generateReport" color="teal" rounded dark
      ...
      <div class="px-5">
        <v-alert
          ...
          prominent
          border="left"
          v-show="Object.keys(transactions).length === 0"
        >
      ...
      <template v-for="(tdate, date) in transactions">
        ...
        <template v-for="(transaction, index) in tdate">
          ...
          >
            ...
            <v-list-item-content class="service">
              ...
              <v-list-item-title class="service-text">{{
                Object.keys(transaction.service).join(', ')
              }}</v-list-item-title>
            </v-list-item-content>
            <v-list-item-content class="amount">
              <v-list-item-subtitle>
                <strong>${{ transaction.price }} </strong>
                <span class="red--text" v-if="transaction.deposit"
                  >(- ${{ transaction.deposit }})</span
                >
              </v-list-item-subtitle>
              <v-list-item-subtitle>
                Tips ${{ transaction.tips }}</v-list-item-subtitle
              >
            </v-list-item-content>
          </v-list-item>
        </template>
        <div :key="date + 'Total'" class="total">
          <v-chip class="ma-2" color="teal" outlined
            ...
            | Tips: ${{
              tdate
                .reduce((acc, current) => ({ tips: acc.tips + current.tips }))
                .tips.toFixed(2)
            }}</v-chip
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    deleted(deleteItem) {
      if (this.rangeTr[deleteItem.date]) {
        if (this.rangeTr[deleteItem.date].length == 1) {
          ...
          this.rangeTr[deleteItem.date] = this.rangeTr[deleteItem.date].filter(
      ...
      if (this.monthTr[deleteItem.date]) {
        if (this.monthTr[deleteItem.date].length == 1) {
          ...
          this.monthTr[deleteItem.date] = this.monthTr[deleteItem.date].filter(
            (item) => item.id !== deleteItem.id
          )
        }
      }
    ...
    listData(querySnapshot, listType) {
      ...
      this.progress = false
      listType === 'Range'
        ? (this.rangeTr = transactions)
        : (this.monthTr = transactions)
    },
    updated() {
      this.updateList()
