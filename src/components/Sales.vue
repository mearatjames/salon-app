          ...
          <v-expansion-panels dark>
            <v-expansion-panel>
              <v-expansion-panel-header
                ...
                >{{
                  `Today Sales: ${new Intl.NumberFormat('en-US', {
                    style: 'currency',
                    currency: 'USD',
                  }).format(today.total)}`
                }}</v-expansion-panel-header
              ...
              <v-expansion-panel-content color="teal darken-1">
                <ul class="today-list">
                  <li v-for="item in today.items" :key="item.name">
                    {{ `${item.name}: ${item.qty} ` }}
                  </li>
                </ul>
...
export default {
  ...
  methods: {
    ...
    back() {
      this.checkout = false
      ;(this.today = { total: 0, items: {} }), this.getSnapshot()
    },
    remove(index) {
        ...
        .then(querySnapshot => {
          querySnapshot.forEach(doc => {
            ...
            data.items.forEach(e => {
              ...
              !this.today.items[e.id]
                ? (this.today.items[e.id] = {
                    name: e.name,
                    qty: e.qty,
                  })
                : (this.today.items[e.id].qty += e.qty)
            })
